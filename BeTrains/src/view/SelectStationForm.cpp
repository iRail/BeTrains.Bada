...
bool SelectStationForm::Initialize() {
	 ...
	 * I18N
	 */
	String empty = "Lege lijst";
	String clear = "Wis";
	String cancel = "Annuleer";
	AppResource* appRes = Application::GetInstance()->GetAppResource();
	appRes->GetString(L"STATION_EMPTY_LIST", empty);
	appRes->GetString(L"STATION_SELECT_CLEAR",clear);
	appRes->GetString(L"STATION_SELECT_CANCEL",cancel);
	...
	editField->Construct(Rectangle(0, 0, bounds.width, itemHeight),
			EDIT_FIELD_STYLE_NORMAL, INPUT_STYLE_OVERLAY, false, 60,
			GROUP_STYLE_NONE);
	...
	editField ->SetOverlayKeypadCommandButton(COMMAND_BUTTON_POSITION_LEFT,
			cancel, CANCEL_ACTION);
	editField->SetOverlayKeypadCommandButton(COMMAND_BUTTON_POSITION_RIGHT,
			clear, CLEAR_ACTION);
	...
	stationSuggestionList->Construct(Rectangle(0, editField->GetHeight(),
			bounds.width, bounds.height - itemHeight), true, false);
	stationSuggestionList->SetTextOfEmptyList(empty);
...
void SelectStationForm::setStation(Station* &selectStation_) {
	editField->SetText("");
	updateList();
...
void SelectStationForm::OnActionPerformed(const Osp::Ui::Control& source,
		int actionId) {
 ...
 */
void SelectStationForm::OnTextValueChangeCanceled(
...
void SelectStationForm::OnListViewItemStateChanged(
...
void SelectStationForm::OnListViewItemSwept(
...
void SelectStationForm::OnListViewContextItemStateChanged(
		Osp::Ui::Controls::ListView &listView, int index, int elementId,
...
int SelectStationForm::GetItemCount(void) {
...
bool SelectStationForm::DeleteItem(int index,
		Osp::Ui::Controls::ListItemBase* pItem, int itemWidth) {
	delete pItem;
	pItem = null;
	return true;
}
...
void SelectStationForm::OnOrientationChanged(const Control& source,
	...
	stationSuggestionList->SetBounds(0, editField->GetHeight(), bounds.width,
