...
void BetrainsPrototype::showSearchStationName() {
	...
	Frame *pFrame = Application::GetInstance()->GetAppFrame()->GetFrame();
	Form *form = pFrame->GetCurrentForm();
	pFrame->AddControl(*pSearchStationNameForm);
	pFrame->SetCurrentForm(*pSearchStationNameForm);
	pSearchStationNameForm->Draw();
	pSearchStationNameForm->Show();
	if (form != null) {
		pFrame->RemoveControl(*form);
	}
}
...
void BetrainsPrototype::showPlannerResults(Osp::Base::String from,
		Osp::Base::String to) {
...
bool BetrainsPrototype::OnAppInitializing(AppRegistry& appRegistry) {
	showMenu();
	return true;
}

bool BetrainsPrototype::OnAppTerminating(AppRegistry& appRegistry,
...
void BetrainsPrototype::OnForeground(void) {
	// TODO:
	// Start or resume drawing when the application is moved to the foreground.
}

void BetrainsPrototype::OnBackground(void) {
	// TODO:
	// Stop drawing when the application is moved to the background.
}

void BetrainsPrototype::OnLowMemory(void) {
	// TODO:
	// Free unused resources or close the application.
}

void BetrainsPrototype::OnBatteryLevelChanged(BatteryLevel batteryLevel) {
	// TODO:
	// Handle any changes in battery level here.
	// Stop using multimedia features(camera, mp3 etc.) if the battery level is CRITICAL.
}

void BetrainsPrototype::OnScreenOn(void) {
	// TODO:
	// Get the released resources or resume the operations that were paused or stopped in OnScreenOff().
}

void BetrainsPrototype::OnScreenOff(void) {
	// TODO:
